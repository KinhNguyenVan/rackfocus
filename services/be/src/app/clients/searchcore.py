...
_stub: pb_grpc.SearchCoreServiceStub | None = None


async def get_stub() -> pb_grpc.SearchCoreServiceStub:
    global _channel, _stub
    if _stub is None:
        _channel = grpc.aio.insecure_channel(TARGET)
        _stub = pb_grpc.SearchCoreServiceStub(_channel)
    return _stub
...
async def health(timeout: float = 2.0):
    stub = await get_stub()
    return await stub.Health(pb.HealthRequest(), timeout=timeout)
