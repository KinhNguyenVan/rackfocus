...
// -------------------------------------------------------------
export function SearchPage({ goSubmission }: { goSubmission: () => void }) {
  ...
  const loadFrames = (
    ...
    setFramesLabel(
      toUrl
        ? `${before} trước · giữa 2 sự kiện · ${after} sau`
        : `${before} trước/sau`,
    );
    ...
    fetchNeighbors(fromUrl, before, after, toUrl)
      ...
      .catch((cause: unknown) => {
        setNeighborFrames([]);
        setNeighborsError(
          cause instanceof Error ? cause.message : "Load frames failed",
        );
      })
      ...
      {contextMenu.visible && contextMenu.target && (
        ...
        >
          ...
          <button
            className="dropdown-item"
            onClick={() =>
              navigator.clipboard.writeText(contextMenu.target!.keyframe_url)
            }
          >
            🔗 Copy URL
          </button>
                ...
                <div className="card-body">
                  ...
                  {searchMode === "temporal" ? (
                    <>
                      ...
                      {temporalError && (
                        <p className="text-danger">{temporalError}</p>
                      )}
                      ...
                      {temporalTotalMs !== null && (
                        <small className="text-muted d-block mt-3">
                          {chains.length} chains · {temporalTotalMs.toFixed(2)}{" "}
                          ms
                        </small>
                      )}
                    </>
      ...
      >
        ...
        {framesOpen && (
          ...
          >
            ...
            {framesLabel && (
              <span className="badge text-bg-primary mb-2">{framesLabel}</span>
            )}
            {neighborsLoading && (
              <div className="text-center my-3">
                <div
            ...
            {!neighborsLoading &&
              ...
              neighborFrames.length === 0 && (
                <div className="text-muted small">
                  Bấm chuột phải vào 1 ảnh kết quả rồi chọn "Xem 25 frames
                  trước/sau".
                </div>
              )}
                  ...
                  <div className="col p-1" key={item.point_id}>
                    <div className="card position-relative">
                      ...
                      <div className="card-body p-1 text-center">
                        ...
                        <small
                          className={
                            nf.is_current ? "fw-bold text-primary" : ""
                          }
                        >
                          #{nf.frame}
                          {nf.is_current ? " · current" : ""}
                        </small>
                      </div>
                    </div>
                  </div>
...
// -------------------------------------------------------------
export function SubmissionPage({ goSearch }: { goSearch: () => void }) {
